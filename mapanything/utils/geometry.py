...
def initialize_rotation_from_gravity(gravity_in_camera):
    """
    Computes an initial camera-to-world rotation matrix from a gravity vector.

    This can be used to initialize a camera's orientation if the direction of
    gravity in the camera's coordinate system is known. It aligns the camera's
    coordinate system such that the sensed gravity aligns with a world "down"
    direction (along negative Y).

    Args:
        gravity_in_camera (np.ndarray): A 3-element numpy array representing the
                                        gravity vector measured in the camera's
                                        coordinate system.

    Returns:
        np.ndarray: A 3x3 rotation matrix for the initial camera pose (R_wc).
    """
    # We need scipy for this. Let's import it here to avoid adding it as a top-level
    # dependency for the whole project if it's only used here.
    try:
        from scipy.spatial.transform import Rotation
    except ImportError:
        raise ImportError(
            "scipy is required for initialize_rotation_from_gravity. "
            "Please install it using: pip install scipy"
        )

    # World gravity vector (Y-down, assuming Y is up in world coordinates)
    gravity_in_world = np.array([0.0, -1.0, 0.0])

    # Normalize the input vector
    g_cam_norm = gravity_in_camera / np.linalg.norm(gravity_in_camera)

    # Find the rotation that aligns the camera's gravity vector with the world's
    # align_vectors expects sequences of vectors
    rot, _ = Rotation.align_vectors([gravity_in_world], [g_cam_norm])
